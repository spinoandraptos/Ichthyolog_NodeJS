...
const addPost = async (request, response) => {
  const jwt_auth = request.get('Authorisation')
  const { userid, username, title, description, uploadTime, sightingLocation, sightingTime, imageURL } = request.body

  try {
    const result = jwt.verify(jwt_auth, process.env.SECRETKEY, { algorithm: 'HS256' });
    db.dbConnect().query('INSERT INTO posts (userid, authorname, title, description, time, sightinglocation, sightingtime, imageurl)\
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
      [userid, username, title, description, uploadTime, sightingLocation, sightingTime, imageURL],
      (error, results) => {
        if (error) {
          throw error
        }
