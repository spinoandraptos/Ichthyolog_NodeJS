          ...
          if (error) {
            ...
            db.dbConnect().query('UPDATE users SET totalposts = totalposts + 1, level = level + 1 WHERE userid = $1'), [userid], (error, result) => {
              if (error) {
                ...
                db.dbConnect().query('INSERT INTO posts (userid, authorname, title, description, time, sightinglocation, sightingtime, imageurl, authorpicurl, class, _order, family, genus, species) VALUES ($1, $2, $3, $4, now(), $5, $6, $7, $8, $9, $10, $11, $12, $13)', 
                  [userid, authorname, title, description, sightingLocation, sightingTime, imageURL, picture, _class, order, family, genus, species], 
                  (error, result) => {
                  if (error) {
                    response.send(error.message)
                  }
